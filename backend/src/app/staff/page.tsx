...
const mockStaff: StaffMember[] = [
  { id: 'usr_01', email: 'admin@omniface.ai', fullName: 'Dr. Anand Raman', role: 'OWNER', status: 'ACTIVE', joinedDate: '2026-06-01', lastLogin: '10 min ago' },
  { id: 'usr_02', email: 'vikram.joshi@nit.edu', fullName: 'Dr. Vikram Joshi', role: 'ADMIN', status: 'ACTIVE', joinedDate: '2026-07-01', lastLogin: '1 hour ago' },
  { id: 'usr_03', email: 'sunita.rao@nit.edu', fullName: 'Prof. Sunita Rao', role: 'TEACHER', status: 'ACTIVE', joinedDate: '2026-07-01', lastLogin: 'Yesterday' },
  { id: 'usr_04', email: 'auditor@state-edu.gov.in', fullName: 'State Education Auditor', role: 'VIEWER', status: 'ACTIVE', joinedDate: '2026-08-15', lastLogin: '3 days ago' },
];
...
export default function StaffPage() {
  ...
  const handleInviteStaff = (e: React.FormEvent) => {
    ...
    setEmail('');
    setFullName('');
    showToast(`Invitation sent to ${email} with ${role} permissions`, 'success');
  };

  const handleRoleChange = (memberId: string, newRole: StaffRole) => {
    setStaffList(staffList.map((m) => m.id === memberId ? { ...m, role: newRole } : m));
          ...
          <tbody>
            {staffList.map((m) => (
              <tr key={m.id}>
                ...
                <td className="tnum" style={{ color: 'var(--text-dim)' }}>{m.joinedDate}</td>
                <td className="tnum" style={{ color: 'var(--text-muted)' }}>{m.lastLogin}</td>
                <td>
                  {m.role !== 'OWNER' ? (
                    <select
                      value={m.role}
                      onChange={(e) => handleRoleChange(m.id, e.target.value as StaffRole)}
