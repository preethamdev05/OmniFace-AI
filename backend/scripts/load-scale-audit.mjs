...
const SECRET = process.env.OMNIFACE_SESSION_SECRET || ('omniface_scale_audit_secret_' + crypto.randomBytes(24).toString('hex'));
...
async function runAudit() {
  ...
  const cohorts = [1, 100, 500, 1000, 2500, 5000, 10000];
  const vectorResults = [];
...
// ── STAGE 3: LIVE NEXT.JS SERVER STARTUP FOR SYNC & API TESTING ──
  ...
  const serverProcess = spawn('npx', ['next', 'start', '-p', String(PORT)], {
    ...
    env: {
      ...
      MIGRATION_SECRET: 'benchmark_migration_secret_secure_9921',
    },
  });
  ...
  try {
    ...
    // Pair a real kiosk device to obtain authoritative database-bound device token
    let deviceToken = 'omni_hw_fallback';
    try {
      const codeRes = await fetchUrl('/api/v1/devices/generate-code', {
        method: 'POST',
        headers: authHeaders,
        body: JSON.stringify({}),
      });
      console.log(`  [DEBUG] generate-code status: ${codeRes.status}, body: ${codeRes.body.slice(0, 100)}`);
      const codeData = JSON.parse(codeRes.body);
      const pairingCode = codeData?.pairingCode;

      const pairRes = await fetchUrl('/api/v1/devices/pair', {
        method: 'POST',
        headers: {
        ...
        body: JSON.stringify({
          pairingCode,
          deviceIdentifier: 'BENCHMARK-KIOSK-' + crypto.randomBytes(4).toString('hex').toUpperCase(),
          deviceName: 'Enterprise Benchmark Terminal',
          hardwareHash: crypto.randomBytes(32).toString('hex'),
        }),
      });
      console.log(`  [DEBUG] pair status: ${pairRes.status}, body: ${pairRes.body.slice(0, 100)}`);
      const pairData = JSON.parse(pairRes.body);
      deviceToken = pairData?.deviceToken || deviceToken;
      console.log('  ✅ Benchmark kiosk registered & paired with deviceToken: ' + deviceToken.slice(0, 16) + '...');
    } catch (pairErr) {
      console.warn('  ⚠️ Benchmark device pairing notice:', pairErr.message);
    }
    ...
    for (const count of syncCohorts) {
      ...
      const eventsPerSec = (count / (latencyMs / 1000)).toFixed(1);
    ...
    for (const dCount of deviceCounts) {
      ...
      for (let dev = 0; dev < dCount; dev++) {
        ...
        const promise = fetchUrl('/api/v1/attendance/sync', {
          ...
          headers: {
            'Content-Type': 'application/json',
            'X-Device-Token': deviceToken,
            'X-Device-ID': `KIOSK-${String(dev).padStart(3, '0')}`,
          },
          body: JSON.stringify({ batchId: `fleet_batch_${dCount}_${dev}`, records }),
        });
      ...
      const overallThroughput = (totalEvents / (totalTimeMs / 1000)).toFixed(1);
    ...
    console.log('\n[6/7] Testing Report Generation & CSV Export Scalability...');
    const reportQueries = [
      { name: 'Daily Aggregation', path: '/api/v1/reports?month=2026-09&department=ALL' },
      { name: 'Monthly Department Breakdown', path: '/api/v1/reports?month=2026-09&department=CS' },
      { name: 'Full CSV Streaming Export', path: '/api/v1/reports/export?month=2026-09&department=ALL' },
    ];
    ...
    console.log('\n--- Terminating Audit Benchmark Server ---');
...
Report Generation Capacity     : PASS (Sub-200ms JSON aggregate, sub-100ms CSV stream)
